...
function Got_Player_Input(MyEvent) {
  switch (game_mode) {
    case "prestart":
      ...
      break;
    case "running":
      bird.velocity_y = reverse_mode ? -jump_amount : jump_amount;
      break;
    case "over":
...
addEventListener("keydown", (e) => {
  if (e.key === "r" || e.key === "R") {
    ...
    myCanvas.style.filter = reverse_mode ? "invert(1)" : "invert(0)";
  }
  Got_Player_Input(e);
});
...
function make_bird_slow_and_fall() {
  if (!reverse_mode) {
    ...
    if (bird.velocity_y > -max_fall_speed) bird.velocity_y -= acceleration;
    if (bird.y < 0 || bird.y > myCanvas.height) {
      bird.velocity_y = 0;
      game_mode = "over";
    }
  }
...
function check_for_end_game() {
  ...
  if (live_score >= 5 && !reverse_mode) {
    reverse_mode = true;
    myCanvas.style.filter = "invert(1)";
  }
}

function display_live_score() {
  ctx.font = "25px Arial";
  ctx.fillStyle = "yellow";
  ctx.textAlign = "left";
  ctx.fillText("Skor: " + live_score, 20, 40);
}
...
function display_intro_instructions() {
  ...
  ctx.fillText("Başlamak için tıklayın.", myCanvas.width / 2, myCanvas.height / 4);
}
...
function display_game_over() {
  ...
  ctx.fillStyle = "red";
  ctx.textAlign = "center";
  ctx.fillText("Maçı Kaybettin!", myCanvas.width / 2, 100);
  ctx.fillText("Skor: " + live_score, myCanvas.width / 2, 150);
  ctx.font = "20px Arial";
  ctx.fillText("Tekrar oynatmak için tıklayın, dokunun veya basın", myCanvas.width / 2, 300);
}
...
function reset_game() {
  ...
  live_score = 0;
  reverse_mode = false;
  myCanvas.style.filter = "invert(0)";
  add_all_my_pipes();
}
...
var bird = new MySprite("adam.png");
